...
5. Vercel Root Directory: repository root / blank (the folder containing index.html, admin.html, vercel.json). Do NOT set it to admin.html or BILZET-main as an extra nested folder. Deploy.
6. Test in an Incognito window: open `/` → Login/Register must be visible. Register/login → dashboard. Refresh → Login screen again. `/` opens the USER portal; `/admin` and `/Admin` open only the ADMIN portal.
